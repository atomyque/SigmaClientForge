A template for 1.8.9 kotlin forge mods based on Deftu's Gradle Toolkit
...
To run the mod you will need two JDKs, one Java 21 jdk and one Java 1.8 jdk. You can download those
from [here](https://adoptium.net/temurin/releases) (or use your own downloads).

When you import your project into IntelliJ, you need to set the gradle jvm to the Java 21 JDK in the gradle tab, and the
Project SDK to the Java 21 JDK. Then click on the sync button in IntelliJ, and it should create a run task
called `Minecraft Client`. Now you need to edit the run task and set the task's jvm to Java 1.8 JDK. If it doesn't then try relaunching your IntelliJ.
